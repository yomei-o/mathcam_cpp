...
| 部品 | 中身 | 状態 |
|---|---|---|
| ① 認識 | 写真 → 記号の検出 → **2 次元レイアウト解析** → 式木 | 合成データは通る（検出 mAP50 0.99、レイアウト解析 100%、端から端まで 95〜98%）。**実写の教科書は 27 問中 20 問**（始まりは 0 / 12。下の「実写はなぜ落ちたか」）。囲まずにページを渡す道は 8 / 27 |
...
```sh
python tools/real_eval.py --dir test_data --model models/sym_det.onnx --show
```
