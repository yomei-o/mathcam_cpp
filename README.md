...
| 部品 | 中身 | 状態 |
|---|---|---|
| ① 認識 | 写真 → 記号の検出 → **2 次元レイアウト解析** → 式木 | **実写の教科書は 32 問中 31 問**（始まりは 0 / 12。下の「実写はなぜ落ちたか」）。合成は端から端まで 297 / 300、組版 → 解析の往復は 200 / 200。囲まずにページを渡す道は 18 / 32 |
| ② 計算 | 式木を書き換えて答えを出す（厳密有理数の CAS） | **高校の教科書に出る形をひととおり**（方程式・不等式・因数分解・三角関数・指数対数・数列と Σ・漸化式・微分積分・極限・面積・複素数解・絶対値・部分分数分解・円の方程式・ベクトル・場合の数）。両言語 + **パリティ 18 スイート**（答えだけでなく手順の全行を突き合わせる） |
| ③ 手順 | 「名前のついた書き換え」を並べて見せる | **完了**（移項・分母を払う・因数分解・解の公式・加減法・代入法・共通範囲・積の微分・合成関数・Σ の公式…） |

**①も②も、C++ と Python の両方でできる。** 長いあいだ検出器の学習だけ Python
（Ultralytics）でしかできなかったが、`mathcam train-det` で塞いだ。損失は Ultralytics の
v8DetectionLoss と一致することをテストで縛ってある。
...
| 関数名をグリフの並びから戻す（sin/cos/tan/ln）+ 下に突き出る字のベースライン + 分数線だけしきい値を下げる | **31 / 32 のまま**（合成の端から端まで 295 → 297 / 300） |
