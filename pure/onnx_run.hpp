...
inline std::map<std::string, Tensor> run_onnx(const Graph& g, const Tensor& x,
  ...
  for (const auto& nd : g.nodes) {
    ...
    if (op == "Constant") {
      ...
      auto dl = attr_ints(nd, "dilations");
      int64_t dil = dl.empty() ? 1 : dl[0];
      // CSRNet's back end is dilation 2, so this is not an exotic path here — it is half the network.
      if (dil > 1)                                           y = conv2d(get(nd.input[0]), w, b, s, ph, grp, dil);
      else if (grp > 1 && grp == w->shape[0] && w->shape[1] == 1) y = dwconv2d(get(nd.input[0]), w, b, s, ph);
