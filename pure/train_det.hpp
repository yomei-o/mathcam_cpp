...
//
// 出どころ: 姉妹リポ yolo_lpr_cpp/pure/train_det.hpp（ナンバープレート検出器 nc=1）。
// **中身はほぼそのまま**で、こちらは nc=39・imgsz 640 になるだけ（クラス数も画像サイズも
// グラフから読むので、書き換えたのは include だけ）。両方を直すときに差分が読めるよう、
// 名前空間 det:: と関数名は向こうに合わせてある。
#pragma once
#include "onnx_train.hpp"
#include "rng.hpp"
#include "trainio.hpp"       // trn::read_file / list_images_recursive、jl::resize_rgb01
#include <array>
#include <cmath>
#include <fstream>
#include <sstream>
...
inline Tensor forward_loss(onx::Trainable& t, const HeadNames& hn, const Tensor& x,
                           ...
                           bool bn_train = false, float bn_momentum = 0.03f) {
  std::set<std::string> stop(hn.box.begin(), hn.box.end());
  stop.insert(hn.cls.begin(), hn.cls.end());
  // A graph written by pure/build_v8.hpp keeps BatchNormalization as its own node and is trained
  // from scratch, so BN runs on batch statistics (momentum 0.03, as ultralytics' initialize_weights
  // sets it). A fused Ultralytics export has no BN nodes at all and this flag does nothing.
  std::map<std::string, Tensor> vals = onx::run_onnx(t.g, x, stop, &t.init, bn_train, bn_momentum);
