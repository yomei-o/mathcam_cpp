...
// `dil` is the dilation (ONNX `dilations`). It is what CSRNet's back end is made of: the receptive
// field grows without another stride, so the density map stays at 1/8 of the input while later layers
// still see a wide context. The only places it enters are the output size and the input index in
// im2col (both forward and in the input-gradient scatter) — everything else is the same GEMM.
inline Tensor conv2d(const Tensor& in, const Tensor& w, const Tensor& bias,
                     int64_t stride, int64_t pad, int64_t groups = 1, int64_t dil = 1) {
  ...
  for (int64_t n = 0; n < N; ++n) {
    ...
    for (int64_t g = 0; g < groups; ++g) {
      im2col_(in->data.data() + (n * Cin + g * Cin_g) * H * W, Cin_g, H, W, kh, kw, OH, OW, stride, pad, colf.data(), dil);
      float* On = O + n * Cout * P;
  ...
  Node* op = o.get();
  o->backward_fn = [in, w, bias, op, N, Cin, H, W, Cout, kh, kw, OH, OW, stride, pad, groups, Cin_g, Cout_g, K, P, dil] {
    const float* Wd = w->data.data();
    ...
    for (int64_t n = 0; n < N; ++n)
      for (int64_t g = 0; g < groups; ++g) {
        ...
        im2col_(in->data.data() + (n * Cin + g * Cin_g) * H * W, Cin_g, H, W, kh, kw, OH, OW, stride, pad, colb.data(), dil);
        ...
        parallel_for(Cin_g, [&](int64_t cig) {
          for (int64_t r = 0; r < kh; ++r)
            for (int64_t s = 0; s < kw; ++s) {
              ...
              for (int64_t oh = 0; oh < OH; ++oh) {
                int64_t ih = oh * stride - pad + r * dil; if (ih < 0 || ih >= H) continue;
                float* girow = GIn + (cig * H + ih) * W;
                for (int64_t ow = 0; ow < OW; ++ow) { int64_t iw = ow * stride - pad + s * dil; if (iw < 0 || iw >= W) continue; girow[iw] += dcrow[oh * OW + ow]; }
              }
            }
        });
      }
