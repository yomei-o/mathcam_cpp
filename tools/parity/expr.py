...
FIXED = [
    ...
    # 円周率（定数）と特別角の三角関数
    "pi", "2pi", "pi/6", "sin(pi/6)", "cos(pi/3)", "tan(pi/4)", "sin(pi)", "cos(pi)",
    "sin(3pi/2)", "sin(2pi/3)", "cos(-pi/4)", "tan(pi/2)", "sin(pi/12)", "2 pi r",
    "sin(x + pi/2)", "sin(pi/6) + cos(pi/3)",
    # 対数（底つき。log(x) は底 10 を明示した形に直す）
    "log(2, 8)", "log(100)", "log(2, 1/8)", "log(4, 8)", "log(2, 3)", "log(x)",
    "log(1, 2)", "log(2, x) + ln(x)", "log(9, 27)",
    # 階乗・順列・組合せ（数学 A）。畳めない大きさは畳まずに残す
    "5!", "0!", "10!", "21!", "C(5, 2)", "P(5, 2)", "C(10, 3)", "C(4, 5)", "3! + 2!",
    "C(6,2)*P(3,2)", "n!", "2*3!", "(2+1)!", "C(n, 2)",
    # 絶対値の書き方（|x - 1| は abs(x - 1) と同じ木にする）
    "|x - 1|", "|x| + |y|", "2|x - 3| + 1", "|-5|", "abs(x - 1)", "|x^2 - 4|",
    # 和の中に和が入る形（"a - (2x + 1)" を "a - 2*x + 1" と書いてはいけない）
    "a - (2x + 1)", "a - 2(x + 1)", "-(x + 1)", "a - (x - 1)",
